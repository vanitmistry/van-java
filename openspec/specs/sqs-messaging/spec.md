# sqs-messaging Specification

## Purpose

Provides sending and receiving of messages on AWS SQS queues (`out-queue` for outbound, `in-queue` for inbound) via AWS SDK v2, configurable to target either real AWS or a local LocalStack instance for hermetic testing.
